...
const input = document.createElement('input');
input.type = 'text';
input.id = 'simulate-input';
const output = document.createElement('div');
output.id = 'simulate-output';
...
test("should simulate keyboard input when only keydown arrives", async ({
  ...
  await expect(page.locator("#simulate-output")).toHaveText("A");
});

test("should preserve focus/blur ordering across multiple inputs", async ({
  page,
}) => {
  const focusScript = `
